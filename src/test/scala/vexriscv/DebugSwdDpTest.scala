...
/**
 * SWD.md step 1b: Phase 2A + 2B (SwdPhyDp = line layer + SW-DP register file), with the
 * stub moved back one layer to the AP seam (SwdApCmd/SwdApRsp — what Phase 2C consumes).
 *
 * The probe side reuses SwdHostDriver unchanged. AP completions are driven explicitly by
 * the test (`apComplete`) so posted-read and WAIT-while-busy semantics are controllable.
 *
