...
/**
 * SWD.md step 2: the complete SWD transport (SwdPhy + SwdDp + SwdDmiGateway with the
 * SWCLK<->debug clock crossing) against a REAL DebugModule with a stubbed DebugHartBus
 * (dmstatus.version/authenticated are static reads — no CPU needed). No LiteX.
 *
